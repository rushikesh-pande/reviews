 ...
 *
 * Usage in service:
 *   @Cacheable(value = "reviewsCache", key = "#id")
 *   @CacheEvict(value = "reviewsCache", key = "#id")
 *   @CachePut(value  = "reviewsCache", key = "#result.id")
 */
@Configuration
@EnableCaching
public class CacheConfig {
    ...
    public CacheManager cacheManager(RedisConnectionFactory factory) {
        ...
        // ── reviews specific caches ─────────────────────────────────────
        caches.put("reviewsCache",       defaultConfig().entryTtl(MEDIUM));
        caches.put("reviewsListCache",   defaultConfig().entryTtl(SHORT));
        caches.put("reviewsCountCache",  defaultConfig().entryTtl(SHORT));
        caches.put("reviewsRefCache",    defaultConfig().entryTtl(LONG));
