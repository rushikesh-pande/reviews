...
-- CREATE INDEX IF NOT EXISTS idx_reviews_status ON reviewss (status);

-- Created date (pagination/sorting by date)
-- CREATE INDEX IF NOT EXISTS idx_reviews_created_at ON reviewss (created_at DESC);

-- Composite: status + created_at (covers most list queries)
-- CREATE INDEX IF NOT EXISTS idx_reviews_status_created
--     ON reviewss (status, created_at DESC);

-- Foreign key index (prevents full-table scan on joins)
-- CREATE INDEX IF NOT EXISTS idx_reviews_user_id ON reviewss (user_id);
